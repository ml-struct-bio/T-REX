"""Regression tests for the 2026-05-31 code-review fixes (F1/F2/F6).

F1: evidence-skip _decision_signature now includes the RECENT-window signals the
    prompt ranks on (per-family su_per_gpu_h_recent / near_miss_yield_recent and
    top-level run_su_count_delta / su_per_gpu_h_recent), so a skip cannot reuse a
    stale plan after recent productivity changes.
F2: diagnostic-lane chained_su_per_gpu_h charges ROUTE-level cost (upstream
    generator gpu_h + downstream structure_refilter gpu_h, including FAILED
    refolds), so it is comparable to a Complexa family's own su_per_gpu_h.
F6: LLMCallRecord.parse_status is normalized to a clean enum token.
"""
...
def test_signature_stable_when_only_lifetime_changes_but_recent_same():
    ...
    # su_per_gpu_h (lifetime) IS in the signature (rounded), so these differ —
    # that's fine; the point of F1 is that the RECENT fields are ALSO present.
    # Assert the recent-equal pair with equal lifetime is identical:
...
def test_chained_recent_is_none_when_lane_went_dry_but_lifetime_high():
    """The whole point of F7: a diagnostic lane that produced chained SU only in
    the PAST shows lifetime chained credit > 0 but recent == None (dry)."""
    ...
    assert mh.chained_su_per_gpu_h_recent is None           # → reads as DRY (the fix)
